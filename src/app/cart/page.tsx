    ...
    <div className="mx-4 mt-5">
      ...
      <div className="flex flex-wrap justify-center">
        {items.map((item, index) => {
          return <ItemCard key={`cart_${index}`} image={item.image} name={item.name} price={item.price}></ItemCard>;
        })}
      </div>
      <div className="flex justify-end my-10">
        ...
        <button className="bg-red-600 text-white px-4 py-2 font-bold hover:bg-red-700 transition duration-300">購入手続きへ</button>
      </div>
    </div>
  );
}
